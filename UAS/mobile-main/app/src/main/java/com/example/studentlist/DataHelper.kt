...
class DataHelper(private val context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null,DATABASE_VERSION){
    ...
    fun addStudent(student: Student):Boolean{
        ...
        values.put(KEY_IPK,student.ipk)
    ...
    fun getAllStudent():ArrayList<Student>{
        ...
        if (cursor.moveToFirst()){
            do{
                ...
                student.ipk = cursor.getString(2)
...
/*
    fun updateStudent():ArrayList<Student>{
        var db = this.writableDatabase
        var vals = ContentValues()
        vals.put(KEY_NIM,student.nim)
        vals.put(KEY_NAME,student.name)
        vals.put(KEY_IPK,student.ipk)
        vals.put(KEY_FAC,student.faculty)
        val success = db.insert(TABLE_NAME,null,values)
        db.close()
        if (success.toInt() == -1){
            Toast.makeText(context,"Terjadi Kesalahan dalam Insert, Coba Lagi",Toast.LENGTH_SHORT).show()
